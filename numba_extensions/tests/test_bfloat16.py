...
from bf16.bf16_bindings import __nv_bfloat16
...
class ProxyTorch(torch.Tensor):
    ...
    def __getattr__(self, attr):
        if attr is "__cuda_array_interface__":
    ...
    def __cuda_array_interface__(self):
        ...
        if self._tensor.is_contiguous():
            ...
            strides = tuple(s * itemsize for s in self._tensor.stride())
        shape = tuple(self.shape)
        data_ptr = self._tensor.data_ptr() if self._tensor.numel() > 0 else 0
        data = (data_ptr, False)  # read-only is false
        return dict(typestr=typestr, shape=shape, strides=strides, data=data,
                version=2)
