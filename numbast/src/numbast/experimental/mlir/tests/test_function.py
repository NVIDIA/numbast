...
def test_mutative_device_function_persists_values(func_bindings, shim_writer):
    ...
    @cuda.jit(link=list(shim_writer.links()))
    def kernel(out):
        ...
        """
        p = ctypes.cast(types.ptr(out), ctypes.POINTER(ctypes.c_int32))
        set_42(p)
        add_one_inplace(p)
        add_one_inplace(p)
...
def test_out_return_device_function_results(_sample_out_functions):
    ...
    creates a kernel that uses `types.ptr` to obtain a pointer for the out parameter, and asserts that:
    - `out_ptr_buf[0] == 9`
    - `out_in_ref[0] == 13`
    """
    ...
    def kernel(out_single, out_pair):
        ...
        """
        out_single[0] = add_out(np.int32(10))
        ret, out = add_out_ret(np.int32(7))
        out_pair[0] = ret
        out_pair[1] = out
    ...
    def kernel_ptr(out_ptr_buf, in_val, out_in_ref):
        ...
        """
        out_ptr = ctypes.cast(
            types.ptr(out_ptr_buf), ctypes.POINTER(ctypes.c_int32)
        )
        add_out_ptr(out_ptr, np.int32(in_val))
        out_in_ref[0] = add_in_ref(np.int32(in_val))
