...
from numba_cuda_mlir import (
    cuda,
)  # use numba-cuda-mlir backend so struct types are in typing context

from numba_cuda_mlir._mlir import ir

from ast_canopy import parse_declarations_from_source
from numbast.experimental.mlir import bind_cxx_structs, MemoryShimWriter
...
def test_struct_binding_has_correct_LLVM_type(sample_structs):
    ...
    target_ctx = mlir_target.target_context

    # Establish an MLIR context when none exists (e.g. when this test runs first).
    with ir.Context(), ir.Location.unknown():
        llvm_ty = target_ctx.get_value_type(nbty)
    assert len(llvm_ty.body) == 3
    ctx = llvm_ty.body[0].context
    assert all(
        ty == ir.IntegerType.get_signless(32, context=ctx)
        for ty in llvm_ty.body
    )
    assert not llvm_ty.packed
