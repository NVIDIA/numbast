...
def implicit_conversion_kernel():
    def _lazy_kernel(globals):
        ...
        def kernel_pass():
            ...
            useFoo(Foo(np.int32(1)))
            useBar(Bar(np.int32(2)))
            useBaz(Baz(Foo(np.int32(3))))

            # call functions such that they require implicit conversion
            useFoo(np.int32(1))
            useBaz(Foo(np.int32(3)))
...
@pytest.mark.xfail(
    reason="Implicit conversion lowering not yet generated for function parameters "
    "whose C++ types have converting constructors (e.g. useFoo(int) via Foo(int)). "
    "Requires struct→function cross-renderer codegen.",
    strict=True,
)
