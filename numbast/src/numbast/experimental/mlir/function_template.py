...
from numbast.experimental.mlir.deduction import deduce_templated_overloads
from numbast.experimental.mlir.intent import ArgIntent, compute_intent_plan
from numbast.experimental.mlir.types import to_c_type_str, to_numba_type
    ...
    class MergedTemplatedFunctionDecl(AbstractTemplate):
        ...
        def generic(self, args, kwds, overloads=overloads, overrides=overrides):
            ...
            if func_plan is None or not func_plan.out_return_indices:
                ...
                if len(param_types) != len(func_plan.visible_param_indices):
                    raise ValueError(
                        "Signature args do not match templated intent plan visible params: "
                        f"sig has {len(param_types)} params but plan expects {len(func_plan.visible_param_indices)}"
                    )
                out_return_map = {
                    orig_idx: out_pos
            ...
            formal_args_str, actual_args_str = (
                _make_templated_function_shim_arg_strings(
                    param_types_inner=param_types_for_shim,
            ...
            # Write shims during typing so they are present before link inputs
            # are consumed at lowering setup time.
