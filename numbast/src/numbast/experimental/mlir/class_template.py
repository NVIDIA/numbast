...
# from numba_cuda_mlir.errors import RequireLiteralValue, TypingError
...
from numbast.experimental.mlir.types import (
    CTYPE_MAPS as C2N,
    to_numba_type,
    is_c_floating_type,
    is_c_integral_type,
...
_UNDEFINED_TYPE = getattr(nbtypes, "undefined", cuda_nbtypes.undefined)
_OPAQUE_TYPE = getattr(nbtypes, "Opaque", cuda_nbtypes.Opaque)
...
class MetaType(sttypes.Type):
    def __init__(self, qualified_template_name):
        super().__init__(
            name=make_unique_type_name(
                self, qualified_template_name + "MetaType"
            )
        )
        self.qualified_template_name = qualified_template_name


class MetaFunctionType(sttypes.Type):
    def __init__(self, template_name):
        super().__init__(
            name=make_unique_type_name(self, template_name + "MetaFunctionType")
        )
...
def bind_cxx_struct_ctor(
    ...
    def ctor_impl(builder, target, args, kws):
        ...
        return ctor_cc(builder, target, args[1:], kws)
...
def bind_cxx_ctsd_ctors(
    ...
    register_global(numba_typeref_ctor, sttypes.Function(TypeRefCallTemplate))
    ...
    if overrides is None:
        ...
        for orig_idx in method_plan.visible_param_indices:
            ...
            if method_plan.intents[orig_idx].value in ("inout_ptr", "out_ptr"):
                param_types.append(sttypes.CPointer(base))
            else:
        ...
        if out_return_types:
            if cxx_return_type == sttypes.void:
                if len(out_return_types) == 1:
                    return_type = out_return_types[0]
                else:
                    return_type = sttypes.Tuple(tuple(out_return_types))
            else:
                return_type = sttypes.Tuple(
    ...
    *,
    ...
    class MergedTemplatedMethodDecl(AbstractTemplate):
        ...
        def generic(
            ...
            lowering_key = (qualname, recvr, param_types, shim_writer)
            if lowering_key not in _TEMPLATED_METHOD_LOWERING_CACHE:
                ...
                if method_plan is None or not method_plan.out_return_indices:
                    param_types_for_shim = tuple(param_types)
                    ...
                    if len(param_types) != len(
                        method_plan.visible_param_indices
                    ):
                        raise ValueError(
                            "Signature args do not match templated intent plan visible params: "
                            f"sig has {len(param_types)} params but plan expects {len(method_plan.visible_param_indices)}"
                        )
                    out_return_map = {
                        orig_idx: out_pos
...
def _make_templated_method_shim_arg_strings(
    *,
    param_types_inner: tuple[sttypes.Type, ...],
    ...
    for i, (nb_ty, cxx_param) in enumerate(zip(param_types_inner, cxx_params)):
        ...
        use_pass_ptr = pass_ptr and isinstance(nb_ty, sttypes.CPointer)
        if use_pass_ptr:
        ...
        if m and is_lref:
            ...
            if not isinstance(nb_ty, sttypes.CPointer):
                raise TypingError(
...
def bind_cxx_class_template_specialization(
    shim_writer: ShimWriterBase,
    struct_decl: ClassTemplateSpecialization,
    instance_type_ref: sttypes.TypeRef,
    aliases: dict[
        str, list[str]
    ...
    *,
    ...
    class S_attr(AttributeTemplate):
        ...
        def _field_ty(self, attr: str) -> sttypes.Type:
            field_ty = public_fields_tys[attr]
            return to_numba_type(field_ty.unqualified_non_ref_type_name)

        def _method_ty(self, typ, attr: str) -> sttypes.BoundFunction:
            template = method_templates[attr]
            return sttypes.BoundFunction(template, typ)

        def _templated_method_ty(self, typ, attr: str) -> sttypes.BoundFunction:
...
def concrete_typing():
    ...
    class ConcreteType(sttypes.Type):
        def __init__(self, meta_type, **targs):
            self.meta_type = meta_type
            self.targs = targs
            super().__init__(
                name=make_unique_type_name(self, self.angled_targs_identifier())
            )

        def angled_targs_identifier(self) -> str:
            """Return a symbol-safe type name for internal compiler identifiers."""
            raw = self.angled_targs_str()
        ...
        def targs_dict_as_c(self):
            ...
            def to_c_str(obj: sttypes.Type | int | float) -> str:
                if isinstance(obj, sttypes.Type):
...
def struct_type_from_instantiation(
    instance: sttypes.Type,
    shim_writer: ShimWriterBase,
    header_path: str,
    *,
    arg_intent: dict | None = None,
) -> tuple[sttypes.TypeRef, ClassTemplateSpecialization]:
...
"""
    ...
    instance_type_ref = sttypes.TypeRef(instance)
...
) -> dict[str, sttypes.Type]:
    def _get_literal_type_cls(c_typ_: str) -> sttypes.Literal:
        ...
        if is_c_integral_type(c_typ_):
            return sttypes.IntegerLiteral
        elif is_c_floating_type(c_typ_):
            return sttypes.FloatLiteral
        else:
            return sttypes.Literal
...
def _split_ctor_and_tparam_kwargs(
    decl: ClassTemplate, kwargs: dict[str, sttypes.Type]
) -> tuple[dict[str, sttypes.Type], dict[str, sttypes.Type]]:
    ...
    """
    ctor_kwargs: dict[str, sttypes.Type] = {}
    tparam_kwargs: dict[str, sttypes.Type] = {}
...
def _bind_ctor_call_args(
    ctor: StructMethod,
    positional_args: tuple[sttypes.Type, ...],
    ctor_kwargs: dict[str, sttypes.Type],
) -> tuple[sttypes.Type, ...]:
    ...
    bound: list[sttypes.Type | None] = [None] * n_params
    ...
    return cast(tuple[sttypes.Type, ...], tuple(bound))
...
def _merge_explicit_and_deduced_targs(
    ...
    for tparam in decl.template_parameters:
        ...
        if deduced_nb_ty is _UNDEFINED_TYPE or isinstance(
            deduced_nb_ty, _OPAQUE_TYPE
        ):
            raise TypingError(
...
def _get_or_bind_concrete_type(
    *,
    ...
    header_path: str,
    arg_intent: dict | None,
) -> tuple[sttypes.TypeRef, ClassTemplateSpecialization, bool]:
...
def _resolve_specialization_ctor(
    *,
    ...
    ctor_call_args: tuple[sttypes.Type, ...] | None = None,
) -> StructMethod:
...
def _ensure_ctor_callconv(
    *,
    specialization_decl: ClassTemplateSpecialization,
    template_ctor: StructMethod,
    ctor_call_args: tuple[sttypes.Type, ...],
    instance_type: sttypes.Type,
    shim_writer: ShimWriterBase,
    ctor_callconv_cache: dict[
        tuple[sttypes.Type, tuple[sttypes.Type, ...]], FunctionCallConv
    ],
) -> tuple[FunctionCallConv, tuple[sttypes.Type, ...]]:
...
def _normalize_ctor_lowering_cache_key(
    instance_type: sttypes.Type,
    call_arg_types: tuple[sttypes.Type, ...],
) -> tuple[sttypes.Type, tuple[sttypes.Type, ...]]:
...
def _register_meta_type(
    ...
    ctor_lowering_cache: dict[
        tuple[sttypes.Type, tuple[sttypes.Type, ...]],
        tuple[FunctionCallConv, int, tuple[sttypes.Type, ...]],
    ],
    ctor_lowering_cache_pysig: dict[
        tuple[sttypes.Type, tuple[sttypes.Type, ...]], str
    ],
    *,
    arg_intent: dict | None = None,
):
    ...
    register_global(stub, sttypes.Function(MetaType_template_decl))
...
def bind_cxx_class_template(
    ...
    *,
    ...
    @lower(TC, sttypes.VarArg(sttypes.Any))
    def lower_ctor_dispatch(
        ...
        return_type = builder.get_numba_type(target)
        call_arg_types = tuple(
            builder.get_numba_type(arg.name) for arg in args
        ) + tuple(builder.get_numba_type(arg.name) for _, arg in kws)
        lowering_key = _normalize_ctor_lowering_cache_key(
            return_type, call_arg_types
        )
        lowered = ctor_lowering_cache.get(lowering_key)
        if lowered is None:
            related_expected_keys = [
                key for key in ctor_lowering_cache if key[0] == return_type
            ]
            if related_expected_keys:
                ...
                logger.debug(
                    ...
                    "(return_type=%s, call_arg_types=%s) is %s. "
            ...
            raise LoweringError(
                ...
                f"{return_type} and args {call_arg_types}."
            )

        flattened_args = tuple(args) + tuple(arg for _, arg in kws)
        ctor_cc, ctor_arg_count, _ = lowered
        return ctor_cc(
            builder,
            target,
            flattened_args[:ctor_arg_count],
            [],
        )
