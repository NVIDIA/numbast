...
def test_void_return_type(decl, impl):
    ...
    @cuda.jit(link=[impl])
    def kernel(arr):
        ptr = ctypes.cast(types.ptr(arr), ctypes.POINTER(ctypes.c_int32))
        set_42(ptr)
...
def test_out_return_function_bindings(decl_out, impl_out):
    ...
    def kernel(out_single, out_pair):
        ...
        """
        out_single[0] = add_out(int32(10))
        ret, out = add_out_ret(int32(7))
        out_pair[0] = ret
        out_pair[1] = out
...
def test_out_ptr_in_inout_function_bindings(decl_out_ptr, impl_out):
    ...
    def kernel(out_ptr_buf, in_val, inout_buf, out_val):
        ...
        add_out(out_ptr, int32(in_val))
        inout_ptr = ctypes.cast(
            types.ptr(inout_buf), ctypes.POINTER(ctypes.c_int32)
        )
        add_inout_ref(inout_ptr, int32(6))
        out_val[0] = add_in_ref(int32(in_val))
