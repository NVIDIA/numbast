...
from numbast.experimental.mlir.types import CTYPE_MAPS
from numbast.experimental.mlir.static.renderer import BaseRenderer
from numbast.experimental.mlir.errors import TypeNotFoundError

_VECTOR_TYPE_PATTERN = re.compile(r"vector\[(\w+),\s*(\d+)\]")


def _vector_symbol_to_vector_str(symbol: str) -> str | None:
    """If symbol is a vector type (e.g. float32x4), return 'vector[elt, n]'; else None."""
    parts = symbol.rsplit("x", 1)
    if len(parts) == 2 and parts[1].isdigit():
        return f"vector[{parts[0]}, {parts[1]}]"
    return None


def _canonical_numba_type_str(nb_type_obj) -> str:
    """Convert a numba type object to a renderable static-binding symbol."""
    type_str = str(nb_type_obj)
    m = _VECTOR_TYPE_PATTERN.match(type_str)
    if m:
        elt, n = m.groups()
        return f"{elt}x{n}"
    return type_str
...
_DEFAULT_CTYPE_TO_NBTYPE_STR_MAP = {
    k: _canonical_numba_type_str(v) for k, v in CTYPE_MAPS.items()
} | {
    "bool": "bool",
    "void": "void",
}
...
def to_numba_type_str(ty: str):
    ...
    vector_str = _vector_symbol_to_vector_str(nb_type_str)
    BaseRenderer._try_import_numba_type(
        vector_str if vector_str is not None else nb_type_str
    )
    return nb_type_str
