...
from numba_cuda_mlir.numba_cuda import types as nbtypes
...
# from numba_cuda_mlir.numba_cuda.cudadecl import register_global, register, register_attr
...
from numba_cuda_mlir.models import StructModel, PrimitiveModel, register_model
...
def bind_cxx_struct_conversion_opeartor(
    ...
    # This is to casting the casted_type to s_type using the conversion operator
    @lower(casted_type, s_type)
    def conversion_impl(builder, target, args, kws):
        return conv_cc(builder, target, args, kws)
    ...
    *,
    ...
    class S_type(parent_type):
        def __init__(self, decl):
            super().__init__(name=make_unique_type_name(self, struct_decl.name))
