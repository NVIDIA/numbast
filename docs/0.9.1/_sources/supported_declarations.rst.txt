...
- ``StructModel`` data layouts support POD fields that are fixed-size arrays,
  multidimensional fixed-size arrays, nested POD structs, and arrays of nested
  POD structs.
...
For ``bind_cxx_struct`` bindings that use ``StructModel``, public POD fields
are modeled with Numba types that preserve C/C++ storage layout:
...
.. list-table::
   ...
   * - ``Inner inner``
     - The bound Numba type for ``Inner``
   * - ``Inner children[3]``
     - ``UniTuple(<Inner numba type>, 3)``

Multidimensional arrays follow C declaration order. For example,
``float transform[2][12]`` is modeled as two rows of twelve ``float`` values,
not twelve rows of two values.
