...
def test_parse_declarations_uses_explicit_clang_binary(monkeypatch, tmp_path):
    source = tmp_path / "sample.cu"
    source.write_text('extern "C" __global__ void kernel() {}\n')
    ...
    monkeypatch.setattr(
        api,
        "check_clang_binary",
        lambda clang_binary=None: clang_binary,
    )
    monkeypatch.setattr(
        api, "get_clang_resource_dir", lambda clang_binary: "/lib/clang/20/"
    )
    ...
    api.parse_declarations_from_source(
        ...
        clang_binary="/opt/llvm/bin/clang++",
    )

    assert captured["command_line_options"][0] == "/opt/llvm/bin/clang++"
